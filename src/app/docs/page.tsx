...
<!-- 3. Initialize -->
<script>
  MultiMesh.init({ container: '#multimesh-widget' })
</script>`,
  },
  {
    label: "Light Theme",
...
const OPTIONS = [
  ...
  { name: "accent", type: "string", desc: "Brand color in hex e.g. '#00E5FF'" },
...
function CodeBlock({ code, lang }: { code: string; lang: string }) {
  ...
  return (
    <div style={{ position: "relative", background: "rgba(6,8,16,0.9)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 12, overflow: "hidden" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 14px", borderBottom: "1px solid rgba(255,255,255,0.05)", background: "rgba(255,255,255,0.02)" }}>
        <span style={{ fontSize: 10, fontFamily: "monospace", color: "#3D4F6B", letterSpacing: 1, textTransform: "uppercase" }}>{lang}</span>
        <button onClick={copy} style={{ fontSize: 10, fontFamily: "monospace", color: copied ? "#00E5FF" : "#3D4F6B", background: "none", border: "none", cursor: "pointer", padding: 0, letterSpacing: 0.5 }}>
          {copied ? "COPIED ✓" : "COPY"}
        </button>
      </div>
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#060810", fontFamily: "'DM Sans', sans-serif", color: "#F0F4FF" }}>
      <style>{`
        ...
        .tab-btn:hover { border-color: rgba(0,229,255,0.2) !important; }
      ...
      <div style={{ position: "fixed", top: -200, right: -200, width: 600, height: 600, background: "radial-gradient(circle,rgba(0,229,255,0.04) 0%,transparent 65%)", pointerEvents: "none" }} />

      {/* Nav */}
      <nav style={{ position: "sticky", top: 0, zIndex: 50, borderBottom: "1px solid rgba(255,255,255,0.04)", background: "rgba(6,8,16,0.95)", backdropFilter: "blur(12px)", padding: "16px 32px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <a href="/" style={{ fontSize: 16, fontWeight: 700, color: "#F0F4FF", textDecoration: "none", letterSpacing: -0.3 }}>
          MULTI<span style={{ color: "#00E5FF" }}>MESH</span>
          <span style={{ fontSize: 10, fontFamily: "monospace", color: "#3D4F6B", marginLeft: 8, letterSpacing: 1 }}>DOCS</span>
        </a>
        <div style={{ display: "flex", gap: 24, alignItems: "center" }}>
          <a href="/" style={{ fontSize: 13, color: "#6B7FA3", textDecoration: "none" }}>App</a>
          <a href="https://t.me/multi_mesh" target="_blank" rel="noopener noreferrer" style={{ fontSize: 13, color: "#6B7FA3", textDecoration: "none" }}>Support</a>
          <a href="/widget" style={{ fontSize: 13, fontWeight: 600, color: "#00E5FF", background: "rgba(0,229,255,0.08)", border: "1px solid rgba(0,229,255,0.2)", borderRadius: 8, padding: "5px 12px", textDecoration: "none" }}>Live Demo →</a>
        </div>
      </nav>
      ...
      <div style={{ maxWidth: 860, margin: "0 auto", padding: "60px 24px 100px", position: "relative", zIndex: 1 }}>
        ...
        <div className="fade-in" style={{ marginBottom: 64 }}>
          <div style={{ fontSize: 11, fontFamily: "monospace", color: "#3D4F6B", letterSpacing: 2, marginBottom: 16 }}>WIDGET INTEGRATION</div>
          <h1 style={{ fontSize: "clamp(32px,5vw,52px)", fontWeight: 700, letterSpacing: -1.5, lineHeight: 1.1, margin: "0 0 20px" }}>
            Embed cross-chain swaps<br />
            <span style={{ color: "#00E5FF" }}>in 3 lines of code.</span>
          </h1>
          <p style={{ fontSize: 16, color: "#6B7FA3", maxWidth: 560, lineHeight: 1.7, margin: 0 }}>
            Add a fully functional cross-chain swap interface to any website. 
            Supports 5 chains, wallet connection, route optimization, and real on-chain execution — out of the box.
          </p>
        </div>
...
<script>MultiMesh.init({ container: '#multimesh-widget' })</script>`} />
        ...
        {/* Live Demo CTA */}
        <div style={{ marginBottom: 56, padding: "24px 28px", background: "rgba(0,229,255,0.04)", border: "1px solid rgba(0,229,255,0.12)", borderRadius: 16, display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 16 }}>
          <div>
          ...
          <a href="/widget" target="_blank" rel="noopener noreferrer" style={{ padding: "10px 22px", borderRadius: 10, background: "#00E5FF", color: "#060810", fontWeight: 700, fontSize: 14, textDecoration: "none", whiteSpace: "nowrap" }}>
            Open Widget Demo →
          </a>
        </div>
        ...
        {/* Examples */}
        <section style={{ marginBottom: 56 }}>
          <h2 style={{ fontSize: 20, fontWeight: 700, marginBottom: 20, letterSpacing: -0.3 }}>Examples</h2>
          <div style={{ display: "flex", gap: 8, marginBottom: 20, flexWrap: "wrap" }}>
            {CODE_EXAMPLES.map((ex, i) => (
              <button key={i} className="tab-btn" onClick={() => setActiveExample(i)}
                style={{ padding: "6px 14px", borderRadius: 8, background: activeExample === i ? "rgba(0,229,255,0.1)" : "rgba(255,255,255,0.03)", border: activeExample === i ? "1px solid rgba(0,229,255,0.3)" : "1px solid rgba(255,255,255,0.07)", color: activeExample === i ? "#00E5FF" : "#6B7FA3", fontSize: 13, fontWeight: 600, cursor: "pointer" }}>
                {ex.label}
              </button>
            ))}
          </div>
        ...
        {/* Config Options */}
        <section style={{ marginBottom: 56 }}>
          <h2 style={{ fontSize: 20, fontWeight: 700, marginBottom: 6, letterSpacing: -0.3 }}>Configuration</h2>
          ...
          <div style={{ border: "1px solid rgba(255,255,255,0.06)", borderRadius: 14, overflow: "hidden" }}>
            ...
            {OPTIONS.map((opt, i) => (
              <div key={opt.name} className="option-row" style={{ display: "grid", gridTemplateColumns: "1fr 1fr 2fr", padding: "12px 16px", borderBottom: i < OPTIONS.length - 1 ? "1px solid rgba(255,255,255,0.04)" : "none" }}>
                <div style={{ fontSize: 12, fontFamily: "monospace", color: "#00E5FF" }}>{opt.name}</div>
        ...
        {/* Supported Chains */}
        <section style={{ marginBottom: 56 }}>
          <h2 style={{ fontSize: 20, fontWeight: 700, marginBottom: 20, letterSpacing: -0.3 }}>Supported Chains</h2>
            ...
            {CHAINS.map(c => (
              <div key={c.id} style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 14px", background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 10 }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: 10, fontFamily: "monospace", color: "#3D4F6B" }}>ID: {c.id}</div>
                </div>
              </div>
            ))}
          </div>
        ...
        {/* Revenue */}
        <section style={{ marginBottom: 56, padding: "24px 28px", background: "rgba(13,17,23,0.95)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 16 }}>
          <h2 style={{ fontSize: 18, fontWeight: 700, marginBottom: 8, marginTop: 0 }}>Fees</h2>
          <p style={{ fontSize: 14, color: "#6B7FA3", lineHeight: 1.7, margin: "0 0 16px" }}>
            MultiMesh charges <strong style={{ color: "#F0F4FF" }}>0.15%</strong> per swap to sustain the protocol. 
            There are no additional integration fees. Interested in a custom revenue sharing arrangement for high-volume partners?
          </p>
          <a href="https://t.me/multi_mesh" target="_blank" rel="noopener noreferrer" style={{ fontSize: 13, fontWeight: 600, color: "#00E5FF" }}>
        ...
        {/* Support */}
        <section>
          <h2 style={{ fontSize: 20, fontWeight: 700, marginBottom: 16, letterSpacing: -0.3 }}>Support</h2>
          <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
            {[
              ...
              <a key={l.label} href={l.url} target="_blank" rel="noopener noreferrer"
                ...
                <div style={{ fontSize: 14, fontWeight: 700, color: "#00E5FF", marginBottom: 4 }}>{l.label} ↗</div>
