  ...
  return (
    <div ref={ref} style={{ position: "relative" }}>
      <button onClick={() => setOpen(o => !o)} style={{ display: "flex", alignItems: "center", gap: 5, background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 8, padding: "4px 8px 4px 6px", cursor: "pointer" }}>
        ...
        <span style={{ fontSize: 11, fontWeight: 600, color: "#A0B0C8" }}>{value.name}</span>
        <span style={{ fontSize: 8, color: "#3D4F6B" }}>▾</span>
      </button>
      {open && (
        <div style={{ position: "absolute", top: "calc(100% + 4px)", right: 0, background: "#0D1117", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 10, padding: 4, zIndex: 100, minWidth: 150, boxShadow: "0 12px 32px rgba(0,0,0,0.7)" }}>
          {SUPPORTED_CHAINS.map(c => (
            <button key={c.id} onClick={() => { onChange(c); setOpen(false); }}
              style={{ display: "flex", alignItems: "center", gap: 8, width: "100%", padding: "7px 8px", background: c.id === value.id ? "rgba(0,229,255,0.06)" : "transparent", border: "none", borderRadius: 7, cursor: "pointer" }}>
              <Img src={CHAIN_LOGOS[c.id]} size={16} />
              <span style={{ fontSize: 12, fontWeight: 600, color: c.id === value.id ? "#00E5FF" : "#E0EAF4" }}>{c.name}</span>
            </button>
          ))}
        </div>
      )}
...
function TokenDropdown({ value, tokens, onChange, chainId }: { value: Token; tokens: Token[]; onChange: (t: Token) => void; chainId: number }) {
  ...
  return (
    <div ref={ref} style={{ position: "relative" }}>
      ...
      {open && (
        <div style={{ position: "absolute", top: "calc(100% + 4px)", right: 0, background: "#0D1117", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 10, padding: 4, zIndex: 100, minWidth: 180, boxShadow: "0 12px 32px rgba(0,0,0,0.7)" }}>
          {tokens.map(t => (
            <button key={t.address} onClick={() => { onChange(t); setOpen(false); }}
              style={{ display: "flex", alignItems: "center", gap: 8, width: "100%", padding: "7px 8px", background: t.address === value.address ? "rgba(0,229,255,0.06)" : "transparent", border: "none", borderRadius: 7, cursor: "pointer" }}>
              <Img src={TOKEN_LOGOS[t.symbol] ?? ""} size={20} />
              <div>
                <div style={{ fontSize: 12, fontWeight: 700, color: t.address === value.address ? "#00E5FF" : "#F0F4FF" }}>{t.symbol}</div>
          ...
          <div style={{ borderTop: "1px solid rgba(255,255,255,0.06)", margin: "4px 0", padding: "6px 4px 2px" }}>
            <div style={{ fontSize: 9, fontFamily: "monospace", color: "#3D4F6B", letterSpacing: 1, marginBottom: 5 }}>PASTE CONTRACT</div>
            <div style={{ display: "flex", gap: 4 }}>
              ...
              <button onClick={handleCustomToken} disabled={customLoading} style={{ padding: "5px 8px", borderRadius: 6, background: "rgba(0,229,255,0.1)", border: "1px solid rgba(0,229,255,0.2)", color: "#00E5FF", fontSize: 10, cursor: "pointer", whiteSpace: "nowrap" }}>
                {customLoading ? "..." : "Add"}
              </button>
            </div>
            {customError && <div style={{ fontSize: 9, color: "#FC8181", marginTop: 3 }}>{customError}</div>}
          </div>
        </div>
      )}
    </div>
...
function WidgetInner() {
  ...
  const integrator = params.get("integrator") ?? "multimesh";
  const accentColor = params.get("accent") ?? "#00E5FF";
  const theme = params.get("theme") ?? "dark";
  const hideBranding = params.get("hideBranding") === "true";

  const isDark = theme !== "light";
  const bg = isDark ? "#060810" : "#F8FAFF";
  const cardBg = isDark ? "rgba(13,17,23,0.98)" : "#FFFFFF";
  const boxBg = isDark ? "rgba(6,8,16,0.8)" : "#F3F6FF";
  const textPrimary = isDark ? "#F0F4FF" : "#0D1117";
  ...
  return (
    <div style={{ minHeight: "100vh", background: bg, display: "flex", alignItems: "center", justifyContent: "center", padding: 12, fontFamily: "'DM Sans', sans-serif", boxSizing: "border-box" }}>
      <style>{`
      ...
      {swap.step !== "idle" && swap.step !== "done" && swap.step !== "failed" && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.8)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 200, padding: 16 }}>
          <div style={{ width: "100%", maxWidth: 360, background: "#0D1117", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 16, padding: 20 }}>
            <div style={{ fontSize: 14, fontWeight: 700, color: "#F0F4FF", marginBottom: 12 }}>Executing Swap</div>
            ...
            {swap.txHash && <div style={{ fontSize: 10, color: "#3D4F6B", fontFamily: "monospace", marginTop: 8, wordBreak: "break-all" }}>{swap.txHash}</div>}
          </div>
        </div>
      )}
      ...
      {swap.step === "done" && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.8)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 200, padding: 16 }}>
          <div style={{ width: "100%", maxWidth: 360, background: "#0D1117", border: `1px solid ${accentColor}44`, borderRadius: 16, padding: 20 }}>
      ...
      {swap.step === "failed" && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.8)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 200, padding: 16 }}>
          <div style={{ width: "100%", maxWidth: 360, background: "#0D1117", border: "1px solid rgba(252,129,129,0.3)", borderRadius: 16, padding: 20 }}>
            <div style={{ fontSize: 14, fontWeight: 700, color: "#FC8181", marginBottom: 8 }}>Swap Failed</div>
            {swap.error && <div style={{ fontSize: 11, fontFamily: "monospace", color: "#A0B0C8", marginBottom: 14, lineHeight: 1.5 }}>{swap.error}</div>}
      ...
      <div style={{ width: "100%", maxWidth: 400 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
          {!hideBranding && (
            <div style={{ fontSize: 14, fontWeight: 700, color: textPrimary, letterSpacing: -0.3 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 6, marginLeft: hideBranding ? "auto" : 0 }}>
            ...
            <div ref={slippageRef} style={{ position: "relative" }}>
              ...
              {showSlippage && (
                <div style={{ position: "absolute", top: "calc(100% + 6px)", right: 0, background: "#0D1117", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 12, padding: 12, zIndex: 150, width: 200, boxShadow: "0 12px 32px rgba(0,0,0,0.6)" }}>
                  <div style={{ fontSize: 9, fontFamily: "monospace", color: "#3D4F6B", letterSpacing: 1, marginBottom: 8 }}>SLIPPAGE</div>
        ...
        <div style={{ background: cardBg, border: `1px solid ${border}`, borderRadius: 18, padding: "12px 11px", backdropFilter: "blur(20px)", boxShadow: isDark ? `0 0 0 1px ${accentColor}08, 0 20px 48px rgba(0,0,0,0.5)` : "0 4px 24px rgba(0,0,0,0.08)" }}>
          ...
          <button onClick={findRoutes} disabled={loading || !amount}
            style={{ width: "100%", padding: 13, borderRadius: 12, background: loading || !amount ? (isDark ? "#0D1520" : "#E8EEF8") : accentColor, color: loading || !amount ? (isDark ? "#2D3F52" : "#8899BB") : "#060810", border: "none", fontWeight: 700, fontSize: 13, cursor: loading || !amount ? "not-allowed" : "pointer", marginTop: 8 }}>
            {loading ? (
          ...
          {(fromTokenUnverified || toTokenUnverified) && (
            <div style={{ fontSize: 10, fontFamily: "monospace", color: "#F3BA2F", padding: "6px 10px", background: "rgba(243,186,47,0.06)", borderRadius: 7, marginTop: 6 }}>
              ⚠ Unverified token — verify contract before swapping.
            </div>
          )}
          {error && <div style={{ fontSize: 11, fontFamily: "monospace", color: "#FC8181", textAlign: "center", padding: 8, background: "rgba(252,129,129,0.06)", borderRadius: 8, marginTop: 6 }}>{error}</div>}

          {/* Swap Now */}
          {!loading && routes.length > 0 && (
            <button onClick={() => { if (address && selectedRoute?.transactionRequest) { swap.execute(selectedRoute); } }}
              style={{ width: "100%", padding: 13, borderRadius: 12, background: accentColor, color: "#060810", border: "none", fontWeight: 700, fontSize: 14, cursor: "pointer", marginTop: 8 }}>
        ...
        {/* Branding footer */}
        {!hideBranding && (
          <div style={{ textAlign: "center", marginTop: 10, fontSize: 10, fontFamily: "monospace", color: isDark ? "#1C2A3A" : "#B0BEC5" }}>
            Powered by <a href="https://multimesh.vercel.app" target="_blank" rel="noopener noreferrer" style={{ color: accentColor, textDecoration: "none" }}>MultiMesh</a> · LI.FI
          </div>
        )}
      </div>
    </div>
