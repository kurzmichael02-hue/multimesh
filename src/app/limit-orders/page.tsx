...
// deBridge DLN chain IDs (different from EVM chain IDs for some)
    ...
    <div ref={ref} style={{ position: "relative" }}>
      ...
      {open && (
        <div style={{ position: "absolute", top: "calc(100% + 4px)", left: 0, background: "#0A0C16", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 12, padding: 6, zIndex: 100, minWidth: 160, boxShadow: "0 16px 40px rgba(0,0,0,0.6)" }}>
          {SUPPORTED_CHAINS.filter(c => c.id !== 11155111).map(c => (
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span style={{ fontSize: 10, fontFamily: "monospace", fontWeight: 700, letterSpacing: 0.5, padding: "3px 8px", borderRadius: 6, background: c.bg, color: c.text }}>
      {status.toUpperCase()}
    </span>
  );
}
...
export default function LimitOrdersPage() {
  ...
  async function getQuote() {
    ...
    try {
      ...
      const dstDecimals = SUPPORTED_TOKENS[dstChain]?.find(t => t.address === dstToken.address)?.decimals ?? 18;
      ...
      if (!dstAmount && data.estimation?.dstChainTokenOut?.recommendedAmount) {
        const recommended = ethers.formatUnits(data.estimation.dstChainTokenOut.recommendedAmount, dstDecimals);
        setDstAmount(parseFloat(recommended).toFixed(4));
      }
    } catch (e: any) {
  ...
  async function submitOrder() {
    ...
    try {
      ...
      await supabase.from("mm_limit_orders").insert({
        ...
        dst_amount: ethers.formatUnits(quote.estimation.dstChainTokenOut.amount ?? quote.estimation.dstChainTokenOut.recommendedAmount, dstDecimals),
        status: "pending",
        tx_hash: tx.hash,
  ...
  const srcChainName = SUPPORTED_CHAINS.find(c => c.id === srcChain)?.name ?? "";
  const dstChainName = SUPPORTED_CHAINS.find(c => c.id === dstChain)?.name ?? "";
    ...
    <div style={{ minHeight: "100vh", background: "#04060E", fontFamily: "'DM Sans', sans-serif", color: "#EEF2FF" }}>
      ...
      <nav style={{ position: "sticky", top: 0, zIndex: 50, borderBottom: "1px solid rgba(255,255,255,0.05)", background: "rgba(4,6,14,0.95)", backdropFilter: "blur(12px)", padding: "0 28px", height: 60, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        <div style={{ display: "flex", gap: 20, alignItems: "center" }}>
          {[{ href: "/", label: "Swap" }, { href: "/limit-orders", label: "Limit Orders" }, { href: "/points", label: "Points ✦" }, { href: "/refuel", label: "Refuel" }].map(({ href, label }) => (
      ...
      <div style={{ maxWidth: 680, margin: "0 auto", padding: "52px 20px 100px", position: "relative", zIndex: 1 }}>
        ...
        <div className="fade-in" style={{ marginBottom: 40, textAlign: "center" }}>
          <div style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "rgba(99,102,241,0.1)", border: "1px solid rgba(99,102,241,0.2)", borderRadius: 100, padding: "4px 14px", marginBottom: 16 }}>
            ...
            <span style={{ fontSize: 11, fontFamily: "monospace", color: "#818CF8", letterSpacing: 0.5 }}>Powered by deBridge DLN</span>
          </div>
          <h1 style={{ fontSize: "clamp(26px,4vw,40px)", fontWeight: 700, fontFamily: "'Space Grotesk',sans-serif", letterSpacing: -1, lineHeight: 1.1, margin: "0 0 14px" }}>
            Cross-Chain Limit Orders
          </h1>
          <p style={{ fontSize: 14, color: "#6B7FA3", maxWidth: 480, margin: "0 auto", lineHeight: 1.7 }}>
            Set your target price and we execute automatically when the market hits it — across any chain. No competitor offers this cross-chain.
          </p>
        </div>
        ...
        {tab === "create" && (
          <div className="fade-in">
            <div style={{ background: "rgba(10,12,22,0.95)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 20, padding: 20 }}>
              ...
              <div style={{ background: "rgba(4,6,14,0.8)", border: "1px solid rgba(255,255,255,0.05)", borderRadius: 14, padding: "12px 16px", marginBottom: 8 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
                  ...
                  <ChainSelect value={srcChain} onChange={id => { setSrcChain(id); setSrcToken({ address: SUPPORTED_TOKENS[id][0].address, symbol: SUPPORTED_TOKENS[id][0].symbol }); setQuote(null); }} />
                </div>
              ...
              <div style={{ background: "rgba(4,6,14,0.8)", border: "1px solid rgba(255,255,255,0.05)", borderRadius: 14, padding: "12px 16px", marginBottom: 16 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
                  ...
                  <ChainSelect value={dstChain} onChange={id => { setDstChain(id); setDstToken({ address: SUPPORTED_TOKENS[id][0].address, symbol: SUPPORTED_TOKENS[id][0].symbol }); setQuote(null); }} />
                </div>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                ...
                <div style={{ fontSize: 11, color: "#4B5A72", marginTop: 8 }}>
                  Leave empty = execute at current market price · Set a number = limit order
                </div>
              </div>
              ...
              {quote && (
                <div style={{ padding: "12px 16px", background: "rgba(99,102,241,0.06)", border: "1px solid rgba(99,102,241,0.15)", borderRadius: 12, marginBottom: 14 }}>
                  ...
                  <div style={{ display: "flex", gap: 20, flexWrap: "wrap" }}>
                    ...
                    <div style={{ fontSize: 18, color: "#4B5A72", alignSelf: "center" }}>→</div>
                    <div>
                      ...
                      <div style={{ fontSize: 14, fontWeight: 700, color: "#818CF8", marginTop: 2 }}>
                        {dstAmount || ethers.formatUnits(quote.estimation.dstChainTokenOut.recommendedAmount, quote.estimation.dstChainTokenOut.decimals).slice(0, 8)} {dstToken.symbol}
                      </div>
                      <div style={{ fontSize: 10, color: "#4B5A72", marginTop: 1 }}>{dstChainName}</div>
                    </div>
                  </div>
                  <div style={{ marginTop: 10, fontSize: 11, color: "#6B7FA3" }}>
                    Order remains open until filled by a solver or cancelled by you. Max validity: 7 days.
                  </div>
                </div>
              )}
              ...
              {!address ? (
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  <button onClick={() => { setQuote(null); setError(""); }} style={{ flex: 1, padding: 14, borderRadius: 12, background: "transparent", color: "#6B7FA3", border: "1px solid rgba(255,255,255,0.08)", fontWeight: 600, fontSize: 14, cursor: "pointer" }}>
                    Edit
                  </button>
            ...
            <div style={{ marginTop: 16, padding: "14px 18px", background: "rgba(99,102,241,0.04)", border: "1px solid rgba(99,102,241,0.1)", borderRadius: 12, fontSize: 12, color: "#6B7FA3", lineHeight: 1.6 }}>
              <strong style={{ color: "#818CF8" }}>How it works:</strong> Your order is submitted on-chain via deBridge's DLN protocol. Solvers compete to fill it — once someone matches your price, the swap executes automatically. You can cancel anytime before it's filled.
            </div>
          </div>
        )}
          ...
          <div className="fade-in">
            {!address ? (
              ...
              <div style={{ background: "rgba(10,12,22,0.9)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 16, padding: "48px 24px", textAlign: "center" }}>
                ...
                <button onClick={() => setTab("create")} style={{ padding: "10px 22px", borderRadius: 10, background: "#6366F1", color: "#fff", fontWeight: 700, fontSize: 13, border: "none", cursor: "pointer" }}>
                  Create Order →
                </button>
              </div>
            ) : (
              <div style={{ background: "rgba(10,12,22,0.9)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 16, overflow: "hidden" }}>
                ...
                {orders.map(o => {
                  const srcChainName = SUPPORTED_CHAINS.find(c => c.id === o.src_chain_id)?.name ?? "";
                  const dstChainName = SUPPORTED_CHAINS.find(c => c.id === o.dst_chain_id)?.name ?? "";
                  return (
                    <div key={o.id} className="order-row" style={{ padding: "14px 20px", borderBottom: "1px solid rgba(255,255,255,0.04)", display: "grid", gridTemplateColumns: "1fr 1fr 1fr auto", alignItems: "center", gap: 8 }}>
                      ...
                      <div>
                        <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                          ...
                          <span style={{ fontSize: 11, color: "#A0B0C8" }}>{srcChainName}</span>
                        </div>
                        <div style={{ display: "flex", alignItems: "center", gap: 4, marginTop: 3 }}>
                          <Img src={CHAIN_LOGOS[o.dst_chain_id]} size={14} />
                          <span style={{ fontSize: 11, color: "#A0B0C8" }}>{dstChainName}</span>
                        </div>
                      </div>
                      <StatusBadge status={o.status} />
                      {o.tx_hash && (
                        <a href={`https://app.debridge.finance/order?orderId=${o.tx_hash}`} target="_blank" rel="noopener noreferrer"
                          style={{ fontSize: 10, fontFamily: "monospace", color: "#818CF8", textDecoration: "none" }}>
                          View ↗
                        </a>
                      )}
                    </div>
